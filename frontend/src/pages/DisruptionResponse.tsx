    ...
    const handleDecision = useCallback(
        (action: "approve" | "edit" | "reject", notes?: string) => {
            if (action === "approve") {
                ...
                toast({
                    title: "Plan Approved",
                    description: "The assignment has been confirmed and notifications sent.",
                });
                ...
                toast({
                    title: "Edit Mode",
                    description: "Edit functionality would allow modifying the plan before confirming.",
                });
            }
        },
        [],
            ...
            <header className="border-b bg-card/80 backdrop-blur-sm sticky top-0 z-30">
                <div className="container flex h-14 items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{scenario.customerSite}</p>
                    </div>
                    {/* Stage progress */}
                    <div className="hidden sm:flex items-center gap-1">
                        {stageLabels.map((_, i) => (
                            <div
                                key={i}
                                className={`h-1.5 w-6 rounded-full transition-colors duration-300 ${i < currentStage ? "bg-primary" : "bg-muted"
                                    }`}
                            />
                        ))}
                    </div>
